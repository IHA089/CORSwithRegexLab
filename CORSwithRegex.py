...
import uuid, datetime, sqlite3, hashlib, random, os, secrets, requests, string, time, re
...
ALLOWED_ORIGINS_REGEX = r".*\.iha089-labs\.in"

CORSwithRegex = Flask(__name__)
CORSwithRegex.secret_key = "vulnerable_lab_by_IHA089"
...
@CORSwithRegex.route('/index.html')
def index_html():
    if not check_cookies():
        session.clear()
    return render_template('index.html', user=session.get('user'))

@CORSwithRegex.route('/login.html')
def login_html():
    if not check_cookies():
        session.clear()
    return render_template('login.html')

@CORSwithRegex.route('/check.html')
def check_html():
    return render_template('check.html')

@CORSwithRegex.route('/join.html')
def join_html():
    if not check_cookies():
        session.clear()
    return render_template('join.html')

@CORSwithRegex.route('/forgot-password.html')
...
@CORSwithRegex.route('/confirm', methods=['POST'])
def confirm():
...
@CORSwithRegex.route('/check', methods=['POST'])
def check():
    username = request.form.get('username')
    session_code = request.form.get('sessioncode')

    if username in flag_data:
        if session_code == flag_data[username]:
            return render_template('success.html', user=session.get('user'))
        else:
            return render_template('check.html', error="wrong session code")
    else:
        return render_template('check.html', error="user not available in flag data")


@CORSwithRegex.route('/resend', methods=['POST'])
def resend():
    username = request.form.get('username')
    password = request.form.get('password')
    hash_password=hashlib.md5(password.encode()).hexdigest()
    ...
    if code:
        ...
        payload = {"email": username,
                    "sender":"IHA089 Labs ::: CORSwithRegexLab",
                    "subject":"CORSwithRegexLab::Verify Your Accout",
...
@CORSwithRegex.route('/login', methods=['GET', 'POST'])
def login():
...
@CORSwithRegex.route('/resetpassword', methods=['POST'])
def resetpassword():
    password=request.form.get('password')
...
@CORSwithRegex.route('/join', methods=['GET', 'POST'])
def join():
    if not check_cookies():
        session.clear()
    if 'user' in session:
...
@CORSwithRegex.route('/reset', methods=['GET'])
def reset():
...
@CORSwithRegex.route('/forgot', methods=['POST'])
def forgot():
    try:
        ...
        if 'username' in data:
            ...
            if result is not None:
                ...
                bdcontent = "<h2>Reset Your Account password</h2><p>Click the button below to reset your account password on CORSwithRegex Lab</p><a href=\""+cmplt_url+"\">Verify Your Account</a><p>If you did not request this, please ignore this email.</p>"
                mail_server = "https://127.0.0.1:7089/dcb8df93f8885473ad69681e82c423163edca1b13cf2f4c39c1956b4d32b4275"
                payload = {"email": username,
                            "sender":"IHA089 Labs ::: CORSwithRegexLab",
                            "subject":"CORSwithRegexLab::Click bellow link to reset your password",
                            "bodycontent":bdcontent
                    }
                try:
                    k = requests.post(mail_server, json = payload)
...
@CORSwithRegex.route('/dashboard')
@CORSwithRegex.route("/dashboard.html")
@login_required
def dashboard():
    ...
    if not origin:
        flag_code = ""
        response = make_response(render_template('dashboard.html', user=session.get('user'), flag=flag_code))
        response.headers.add("Access-Control-Allow-Origin", "*")
        return response
    if re.match(ALLOWED_ORIGINS_REGEX, origin):
...
@CORSwithRegex.route('/logout.html')
def logout():
    session.clear() 
...
@CORSwithRegex.route('/profile')
@CORSwithRegex.route('/profile.html')
@login_required
def profile():
    if not check_cookies():
        session.clear()
...
@CORSwithRegex.route('/reset_password', methods=['POST'])
def reset_password():
    username = request.form.get('username')
